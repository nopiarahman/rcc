<div class="max-w-2xl">
    <div class="flex items-center gap-3 mb-6">
        ...
        <flux:heading size="xl">Tambah Botolan Minuman</flux:heading>
    </div>
    ...
    <div class="space-y-5">
            ...
            <select wire:model="minumanId"
                ...
                @foreach($minumans as $minuman)
                    <option value="{{ $minuman->id }}"
                        {{ in_array($minuman->id, $configured) ? 'disabled' : '' }}>
                        {{ $minuman->nama }}{{ in_array($minuman->id, $configured) ? ' (sudah dikonfigurasi)' : '' }}
                    </option>
                @endforeach
            </select>
        ...
        <flux:field>
            ...
            <p class="text-xs text-zinc-500 mb-1">Upload foto minuman dalam kemasan botol (wajib untuk tampilan halaman publik)</p>
